...
import { Clock, Timer, X, Calendar } from 'lucide-react';
...
export default function HabitForm({ initialData }: HabitFormProps) {
  ...
  const [color, setColor] = useState(initialData?.color || HABIT_COLORS[0].value);
  const [icon, setIcon] = useState<HabitIconId | null>(
    initialData?.icon ? (HABIT_ICONS.find(i => i.id === initialData.icon)?.id || null) : null
  );
  const [frequency, setFrequency] = useState(initialData?.frequency || 'daily');
  const [customDays, setCustomDays] = useState<number[]>(
    initialData?.custom_days ? JSON.parse(initialData.custom_days) : []
  );
            ...
            return (
              <div key={opt.id} className="relative group">
                <button
                  ...
                  className={`w-full aspect-square flex items-center justify-center rounded-xl border transition-all duration-200 ${
                    isSelected
                      ? 'bg-primary text-white dark:text-background-dark border-primary shadow-md scale-105'
                      : 'bg-white dark:bg-white/5 border-gray-200 dark:border-white/10 text-gray-500 dark:text-white/60 hover:border-primary/50 hover:text-primary dark:hover:text-white hover:bg-gray-50 dark:hover:bg-white/10'
                  }`}
                >
                  <SelectedIcon size={20} strokeWidth={isSelected ? 2.5 : 2} />
                </button>
                
                {/* Tooltip */}
                <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 px-2 py-1 bg-gray-800 dark:bg-white text-white dark:text-gray-900 text-xs rounded opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none whitespace-nowrap z-10 shadow-lg">
                  {opt.label}
                  <div className="absolute top-full left-1/2 -translate-x-1/2 border-4 border-transparent border-t-gray-800 dark:border-t-white"></div>
                </div>
              </div>
            );
          })}
